...
1. FunSearch方法与其他方法对比

| 数据集    | 观测上限（不含约束条件） | 贪心算法    | OPRP方法  | LMEA方法 | FunSearch方法 |
|--------|--------------|---------|---------|--------|-------------|
| CHN-24 | 54.45%       | 45.63%  | *       | *      | **46.051%** |
| CHN-12 | 39.6%        | 26.97%  | *       | *      | **27.05%**  |
| CHN-6  | 28.14%       | 17.172% | 17.172% | **     | 17.172%     |
| CHN-3  | 11.67%       | 7.299%  | 7.299%  | 7.299% | ***         |

* *数据量太大，超过api接受token上限
* ** 大模型似乎无法理解，返回的规划方案均为无效方案
* *** 未进行实验

表中百分比为规划方案覆盖率，越大越好

2. FunSearch方法迭代后算法效果

| 数据集    | Baseline* | 贪心算法   | FunSearch方法 |
|--------|-----------|--------|-------------|
| CHN-24 | **        | 5.326  | **5.739**   |
| SEA-24 | **        | 87.414 | **96.037**  |
| PAC-24 | **        | 30.456 | **35.932**  |

表中记录的是最后生成规划方案覆盖率与规划时间的比值，由于覆盖率越大越好，规划时间越小越好，因此整体该比值越大越好

* *Baseline为传给FunSearch的初版程序
* ** 由于初版程序并非完整算法（在程序上是无法跑通的），因此其覆盖率为0，且没有规划时间，没有有效比值

![](scores_list_2024-03-10-18-19-21.png)